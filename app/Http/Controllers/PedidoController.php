...
class PedidoController extends Controller
{
    ...
    public function create()
    {
        ...
        $maquinas = Maquina::all()->map(function ($maquina) {
            $marca = $maquina->marcas->first(); // Obtener la primera marca asociada
            $maquina->marca_nombre = $marca ? $marca->nombre : ''; // Agregar el nombre de la marca al objeto maquina
            return $maquina;
        });
        // dd($maquinas);
        // Obtener todas las marcas únicas asociadas a las máquinas
        $marcasUnicas = $maquinas->pluck('marca_nombre')->unique();
        ...
        return view('pedidos.create')->with('ultimoPedido', $ultimoPedido)->with('usuario', $usuario)->with('Terceros', $Terceros)->with('paises', $paises)->with('maquinas', $maquinas)->with('sistemas', $sistemas)->with('articulos', $articulos)->with('marcas', $marcas)->with('marcasUnicas', $marcasUnicas);
    }
